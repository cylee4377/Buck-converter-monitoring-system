...
## 목차
1. [시스템 개요](#1-시스템-개요)
2. [하드웨어 구성 및 측정 원리](#2-하드웨어-구성-및-측정-원리)
3. [시스템 아키텍처 및 데이터 흐름](#3-시스템-아키텍처-및-데이터-흐름)
   - [Phase A: 엣지(Edge) 단계](#phase-a-엣지edge-단계--esp32)
   - [Phase B: 클라우드(Cloud) 단계](#phase-b-클라우드cloud-단계--aws)
   - [Phase C: 프론트엔드(Frontend) 단계](#phase-c-프론트엔드frontend-단계--aws-amplify)
4. [핵심 성공 요인 (CSF)](#4-핵심-성공-요인-critical-success-factors)

---

## 1. 시스템 개요

본 프로젝트는 **DC-DC 벅 컨버터(Buck Converter)**의 전력 변환 효율($\eta$)을 **실시간**으로 측정하고, 이를 **AWS 클라우드** 파이프라인을 통해 웹 대시보드에 시각화하는 IoT 시스템입니다.
...
벅 컨버터의 효율은 **출력 전력 대비 입력 전력의 비율**로 정의됩니다.

$$\eta = \left( \frac{P_{out}}{P_{in}} \right) \times 100 = \left( \frac{V_{out} \times I_{out}}{V_{in} \times I_{in}} \right) \times 100 \ [\%]$$
...
|-----------|------|----------|--------|-----------|
| **Input Side** | INA260 #1 | `0x40` (A0=GND, A1=GND) | $V_{in}$, $I_{in}$ | $P_{in} = V_{in} \times I_{in}$ |
| **Output Side** | INA260 #2 | `0x41` (A0=VCC, A1=GND) | $V_{out}$, $I_{out}$ | $P_{out} = V_{out} \times I_{out}$ |
...
| **② 연산** | $\eta = \frac{V_{out} \times I_{out}}{V_{in} \times I_{in}} \times 100$ 계산 |
...
| **Amazon DynamoDB** | NoSQL DB. `timestamp`를 파티션 키(Partition Key)로 사용하여 시계열 데이터 적재 |
...
> ESP32가 AWS IoT Core에 연결하려면 **X.509 인증서 기반 TLS 상호 인증**이 필수입니다. 인증서 관리, IoT Policy, 올바른 MQTT Topic 구조 설계가 잘못될 경우 클라우드 연결 자체가 불가능해집니다. 또한 Lambda 실행 역할(IAM Role)의 DynamoDB 쓰기 권한 설정이 누락되면 데이터가 유실됩니다. **IAM 권한 최소 원칙(PoLP)**을 지키며 단계별로 연결을 검증해야 합니다.
